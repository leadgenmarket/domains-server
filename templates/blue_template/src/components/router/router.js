...
const Router = () => {
    ...
    const getBackground = () => {
        if (isMobile()) {
            return params.background_mobile
        }
        return params.background
    }

    return <div className="container_main" style={{ background: params.background != "" ? `url("/file-store/${getBackground()!=""?getBackground():params.background}") center / cover no-repeat` : `` }}>
