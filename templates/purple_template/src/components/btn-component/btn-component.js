...
const BtnComponent = ({ params, clickFunct, text, analog }) => {
    ...
    if (analog) {
        return <button className="btn_main" style={{ color: `#463db0` }} onClick={clickFunct} >{text}</button>
    }
    return <a href="#" class="btn_main fixed_btn step_btn" onClick={clickFunct}>{text}</a>
}
/*
