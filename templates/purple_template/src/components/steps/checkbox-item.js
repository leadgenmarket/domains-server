...
const CheckBoxItem = ({ step, form, setForm, checked, updateCheck, index, params, nextStep, prevStep, answer }) => {
  return <li key={answer + "_wr"} onClick={() => { updateCheck(answer) }} style={{ position: "relative", fontSize: "22px", color: "#FFF", lineHeight: "10px" }} className={checked ? "locs_li act" : "locs_li"}>
    <span>{answer}</span>
  </li>
}
